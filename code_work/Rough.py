...
#definig the df as a global variable
df = None
df_original = None
df_1 = None
...
async def generate_graph(request):
    global finalCols
    # Generate the graph (example using matplotlib)
    ...
    plt.bar(null_frequencies.index, null_frequencies.values, width = 0.3, color = "y")
    plt.xlabel('Columns')
...
#-------First focusing to fill the null values of state, pincode, district, latitude and longitude values of a df if these columns are present 
    ...
    if ('latitude' and 'longitude') in df.columns:
        if 'pincode' in df.columns or 'address' in df.columns :
            df = impute_lat_long(df)
     
    #---function to impute missing pincode, state, district values 
    def check_and_impute_pin_dist_state(data):
        """
        ...
        for index, row in df.iterrows():
            ...
            if pd.isnull(pincode):  # Check if pincode is null
                ...
                if extracted_pincode:  # Check if pincode is found in address
                    df.at[index, 'pincode'] = extracted_pincode[0]  # Fill pincode cell with extracted pincode
        
        return df

    if ("pincode" and 'address') in finalCols:
        df = fill_pincode(df)
    ...
    #Int the last fill the none using avg and most frequent
    df_1 = fillGenColumns_null(df)
...
async def get_data(request):
    ...
    data = df_1.to_dict(orient='records')  # Convert DataFrame to JSON
